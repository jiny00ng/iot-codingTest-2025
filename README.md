...
#### 시간 복잡도 표기법
- 빅오 표기법
- 수학에서 1,2,3,4,5,... n 이라고 할때 n을 의미
- n 대신 x로 표현할 수도 있음

|순번|시간복잡도|최대연산 횟수(1초당)|비고|
|--:|:--|--:|:--|
|1|$O(\log n)$|1,000,000,000|단일 for문 중간 연산을 빠지는 경우가 매우 많은 경우 (예: 369)|
|2|$O(n)$|10,000,000|for문 반복문 사용 / 반복문 하나 사용!|
|3|$O(n \log n)$|1,000,000|2중 for 문에서 반복문 하나가 log n 횟수로 연산|
...
#### 시간 복잡도 계산
- 실제 별찍기는 $O(n)$. 하지만 수학적으로 계산해보면 $O(n^2)$로 나올 수 있음
- 박테리아 계산도 16개를 매년 하나씩 줄어들면 16년이 걸릴 것을 예상(n)
    - 반씩 줄면 횟수가 더 많이 주는 것을 유추할 수 있음 => $O(\log n)

### 코딩 파이썬 필수 문법
[노트북](./day01/02_필수문법.ipynb)

## 2일차 - 코테 정복시작

### 배열
- 다른 언어와 달리 파이썬은 배열이 없음! 리스트를 사용해서 배열로 대체 사용
- [노트묵](./day02/01_배열.ipynb)
...
### 모의 테스트
1. [두개 뽑아서 더하기](https://school.programmers.co.kr/learn/courses/30/lessons/68644)
...
[노트북](./day02/02_스택.ipynb)

## 3일차 - 코테 정복 시작

### 깃헙 팁
- https://github.com/jiny00ng 주소에 com -> dev
...
### 스택
- 몸풀기 문제, 모의 테스트
- 제출 후 체점하기에서 print()문 제거 또는 주석 처리할 것!(효율성 테스트)
- [노트북](./day03/01_스택.ipynb)

### 큐
- FIFO : 맨 먼저 상입한 데이터가 가장 먼저 출력되는 구조
- enQueue() : 데이터 삽입.
- deQueue() : 데이터 추출
- [노트북](./day03/02_큐.ipynb)
...
- 딕셔너리와 동일. 키와 값의 쌍
    ...
    <img src="./image/ct0005.png" widh="500">
...
- 고전적으로 나눗셈법, 곱셈법, 문자열해싱 등을 사용해서 해시함수 구현
- UUID : 32자리 16진수 char + 4개 '-' 를 포함한 구조. 해시를 통해서 생성
- 해시함수에서 다른 키에 대해서 같은 인덱스의 충돌이 발생 할 수 있음 

#### 문제풀이
-[노트북](./day04/01_해시.ipynb)
